...
/**
 * relocateInstruction() 方法是 inline hook 的关键之一
 * 其作用是将原始函数的被跳转指令替换的那几个指令拷贝到 trampoline_instructions 中，
 * 此时 PC 值已经变动，所以还需要对相关指令进行修正
 * PC 值相关的指令由 INSTRUCTION_TYPE 描述
 ...
 * @param orig_boundaries			存放原有指令的指令边界（所谓边界即为该条指令与起始地址的偏移量），用于后续
 * @param trampoline_boundaries		存放修正后指令的指令边界，用途与上相同
 * @param count						处理的指令项数，用途与上相同
 */
